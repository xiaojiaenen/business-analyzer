...
src/
  pages/
    BusinessOverview.tsx   # 页面组件（主 Agent 拥有）：import + 排序各 Section
  sections/
    business-overview/
...
- 每个 `sections/<doc-name>/NN-*.tsx` 导出一个组件，内部用 `<Section index="NN" title="...">…</Section>`。
- 页面组件只做组装：
  ```tsx
  import { Article, Hero, Lead, Conclusion } from "reacticle";
  import { SectionOpening } from "../sections/business-overview/01-opening";
  import { SectionContext } from "../sections/business-overview/02-context";
...
- **全局序号归主 Agent（assembler）所有。** 在页面组件中排定最终顺序后，主 Agent 据此把每个
...
- **subagent 不自编全局序号。** 主 Agent 在派活时直接告诉它"你是第 `<NN>` 章"，subagent 用这个
  ...
  Agent 统一过一遍。
- **组装后必校验**：对照 TOC 显示与 `plan/plan.md` 的 Outline 段顺序，确认序号连续单调、子节前缀正确（并入终审
...
subagent 各**拥有一个** `sections/<doc-name>/NN-*.tsx` 并行开发。**主 Agent 负责合并与稳定性**：

- 维护页面组件的 import 与 Section 顺序（唯一组装点，避免冲突），并据最终顺序**统一校准每个
...
不要修改页面组件（主 Agent 统一组装与序号校准），不要改主题。
```
