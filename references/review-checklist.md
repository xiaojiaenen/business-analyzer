...
> 本文件的核心目的：**让每个节点用对的方式做对的事**，不要错开 SubAgent、不要错写 review 文件。
> 误开 SubAgent / 错写 review 文件是首要性能问题。完整规则见 SKILL.md「硬性质检协议」段（Phase 4 完成标准之后）。

## 各阶段质检方式（铁律）

> 本 skill 的 Phase 编号见 SKILL.md：Phase 1 扫描 / Phase 2 深潜 / Phase 3 结构化（Checkpoint 1）/ Phase 4 生成（Checkpoint 2、Section、终审）/ Phase 5 交付。

| 阶段 | 质检方式 | 产物 |
|---|---|---|
| **Phase 3 Plan / Checkpoint 1 前** | **主 Agent 内联自查（禁开 SubAgent）** | **无文件** |
| **Phase 4 First Spread / Checkpoint 2 前** | First Spread Reviewer SubAgent | `review/first-spread-review.md` |
| **Phase 4 每个 Section** | Section Reviewer SubAgent | **以消息返回 pass/fail + 修复点，不写文件** |
| **Phase 4 终审（进 Phase 5 前）** | Editorial + Visual + Technical Reviewer SubAgent | `review/final-review.md` |

拿到结论后**先按 fail 项把产出改完，再向用户汇报**。直接拿结论汇报但不修复 = 违规。

---

## Plan 自查（Phase 3 → Checkpoint 1 · 主 Agent 内联 · 5 条）
...
2. **信息取舍有据**：每个"可删减"项都给出了理由（重复 / 旁枝 / 过时）；每个"必须保留"项
   都指向 `analysis/business-knowledge.md` 的具体段落 / 表格 / 代码。
...
SubAgent 读页面组件（如 `src/pages/BusinessOverview.tsx`）、`src/sections/<doc-name>/01-*.tsx`、`src/components/Cover.tsx`（若有）、
`plan/plan.md`、选定主题 `references/themes/<id>.md`，按清单核查，写 `review/first-spread-review.md`：
...
```text
请作为 First Spread Reviewer。读取 src/components/Cover.tsx（若有）、页面组件（如 src/pages/BusinessOverview.tsx）、
src/sections/<doc-name>/01-*.tsx、plan/plan.md、references/themes/<id>.md、references/cover.md
...
## Section 自检清单（Phase 4 每个 Section · SubAgent · 消息返回）

SubAgent 读对应 `sections/<NN>-*.tsx`、`plan/plan.md` 的本节段落、`analysis/business-knowledge.md` 本节
...
```text
请作为 Section Reviewer。读取 src/sections/<doc-name>/<NN>-*.tsx、plan/plan.md 本节段落、
analysis/business-knowledge.md 本节对应内容、references/themes/<id>.md。
...
## Phase 4 终审三视角（SubAgent · 写 `review/final-review.md`）
...
- `npm run build` 可构建，`dist/index.html` 可打开、可分享。
...
```text
请作为 <Editorial / Visual / Technical> Reviewer。读取 plan/plan.md、analysis/business-knowledge.md、
页面组件和所有 src/sections/<doc-name>/*.tsx、references/themes/<id>.md。
