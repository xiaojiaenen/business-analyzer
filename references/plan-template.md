...
- 目标读者：<谁会读，带着什么问题>
- 目标语言：<跟随 README 语言（默认） / 指定语言>。事实底座是 `analysis/business-knowledge.md`（Phase 2 已写入）。
...
- 必须保留的信息：<章节 / 表格 / 代码 / 数据 / 引用，逐条列；指向 analysis/business-knowledge.md 的具体位置>
...
1. <编号 NN> <标题>
   - 保留信息：<从 analysis/business-knowledge.md 哪几段，要保留到什么程度>
...
- 当前信息密度下的表现建议：<正文 / Raw / 图片比例如何调整；可引用 references/themes/<id>.md>
...
- **Outline 是章节锚点**：Section subagent（开发模式 B 下）会读这一段找到自己负责的章节。
  每节用 `<编号 NN> <标题>` 起头，编号要和最终 `src/sections/<doc-name>/NN-*.tsx` 一致。
...
- 封面（书封式题图：屏幕 3:4 / PDF 独占首页）：见 `cover.md`，结论落到 **Brief** 段的
  "封面"一行；正式视觉在 Phase 4 First Spread 写 `src/components/Cover.tsx` 时定稿。
