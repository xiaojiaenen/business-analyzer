...
## 分析模式开关（Phase 1 检查点 0 让用户选）

本 skill 有两种分析模式，**Phase 1 开头必须问用户选哪种**（除非用户在触发时已明示）：

### 模式 A · 重点深挖模式（默认）

**适用场景**：给新人入门、给业务方讲解、做项目交接的常规业务文档。

**特点**：
- 核心 5-10 个业务概念深挖（定义+关系网+枢纽实体识别）
- 核心实体只列 3-5 个最重要的业务属性（不列全字段）
- 端到端主线 1-3 条深挖（每条 3-7 子流程 + 异常分支）
- 非核心实体在词汇表/全景图一句话带过
- 文档精简，3 小时可读完

**触发词**：用户说"分析业务""了解业务""业务文档""新人上手"时默认用此模式。

### 模式 B · 全量深挖模式

**适用场景**：系统审计、业务资产盘点、对外交付的完整业务规格书、法律合规留档、内部知识库全量建档。

**特点**：
- **每个业务实体**都深挖（不限 5-10 个核心）：定义 + 全字段业务翻译 + 关键属性 + 生命周期 + 关系
- **每个业务能力**都展开（不限核心域）：每个操作含触发条件 + 执行步骤 + 状态变化 + 异常处理
- **每条端到端主线**都深挖（不限 1-3 条）：识别所有主线，每条都画子流程 + 状态机 + 异常分支
- **每个状态机**都画（不限核心实体）：所有有 status 字段的实体都画状态机
- **每条业务规则**都列（不限 5 条）：8 路全扫的所有规则都进文档
- **每个异常分支**都列（不限 3-5 条）：4 路覆盖率校验的所有异常都进文档
- 文档详尽，可能 50-100+ 页，作为业务规格书长期参考

**触发词**：用户说"全量深挖""完整业务规格""业务审计""全量建档""每个业务都要分析""不要漏任何细节"时用此模式。

**全量深挖的文档膨胀应对**：
- 复杂度总分 ≥ 8 的项目，**强制按业务域拆分文档**（每域 1 份流程 + 1 份规则 + 1 份状态机）
- 单份文档超过 30 个 section 时，拆分为上/下册
- 词汇表按业务域分组（每个域一个折叠区块）

### 模式选择在 Checkpoint 0（Phase 1 开头）

Phase 1 问完 5 个元问题后、用户确认业务理解后，加一个 Checkpoint 0：

> 🛑 **检查点 0 · 分析模式选择**
>
> 我已理解业务。接下来分析有两种模式：
> - **A · 重点深挖**（推荐，3 小时可读完）：核心 5-10 个概念深挖，非核心带过。适合新人入门、项目交接。
> - **B · 全量深挖**（详尽，可能 50+ 页）：每个业务都深挖，不漏任何细节。适合业务审计、全量建档。
>
> 默认 A。请选 A 或 B。

用户选 B 后，所有"只列核心""只挑 5-10 个""一句话带过"的限制都解除，按全量标准执行。
...
- [ ] **Checkpoint 0 已完成**：用户已选分析模式（A 重点深挖 / B 全量深挖），并写入 `plan/plan.md`
- [ ] 复杂度判断已做出（见下方「项目复杂度自动判断标准」，简单 → 1-3 份 / 中等 → 4-6 份 / 复杂 → 7+ 份或按业务域拆分）
- [ ] **如果是微服务架构**，已完成 Phase 1.5 服务发现

#### 项目复杂度自动判断标准（Phase 1 末尾必做，AI 自动评分）

**不要凭感觉判断复杂度**。用以下 6 个量化指标评分，每项 0-2 分，加总后按阈值判断：

| 指标 | 0 分 | 1 分 | 2 分 |
|------|------|------|------|
| **端到端主线数** | 1 条 | 2-3 条 | 4+ 条 |
| **业务实体数**（来自数据库表/代码 model） | ≤ 10 | 11-30 | 31+ |
| **业务域数**（来自 §2.5 领域划分） | 1-2 个 | 3-4 个 | 5+ 个 |
| **微服务数**（来自 Phase 1.5 服务发现） | 1 个（单项目） | 2-3 个 | 4+ 个 |
| **核心实体状态机复杂度**（最大状态数） | ≤ 3 个状态 | 4-7 个状态 | 8+ 个状态 |
| **入口点总数**（HTTP+CLI+MQ+定时+Webhook+后台） | ≤ 15 | 16-40 | 41+ |

**总分阈值**：

| 总分 | 复杂度 | 文档策略 | 默认文档数 |
|------|--------|---------|-----------|
| 0-3 | **简单** | 单项目单文档组 | 3-5 份 |
| 4-7 | **中等** | 单项目完整文档组 | 5-8 份 |
| 8-12 | **复杂** | **按业务域拆分多份流程文档** + 完整文档组 | 8-15 份 |

**复杂项目的自动拆分规则**（总分 ≥ 8 分时触发）：

1. **核心业务流程按业务域拆分**：每个业务域 1 份流程文档 + 1 份跨域总览
   ```
   文档 3a：[业务域 A]业务流程   ← 该域的端到端主线
   文档 3b：[业务域 B]业务流程   ← 该域的端到端主线
   文档 3c：[业务域 C]业务流程   ← 该域的端到端主线
   文档 3d：跨域主线总览         ← 所有主线全景 + 跨域交叉关系 + 跨域异常补偿
   ```
2. **业务规则手册按业务域拆分**（当规则数 > 20 条时）：每域 1 份规则文档
3. **状态机手册按业务域拆分**（当核心实体 > 5 个时）：每域 1 份状态机文档
4. **其他文档保持单份**（业务全景图、领域模型、词汇表、系统架构、角色权限）

**判断结果写入 `plan/plan.md` 开头**：
```markdown
## 项目复杂度评估

| 指标 | 得分 | 依据 |
|------|------|------|
| 端到端主线数 | 2 | 4 条（P2P/O2C/I2O/R2R） |
| 业务实体数 | 2 | 35 张表 |
| 业务域数 | 2 | 5 个域（采购/销售/库存/财务/风控） |
| 微服务数 | 2 | 6 个服务 |
| 状态机复杂度 | 1 | 最大 7 个状态（订单） |
| 入口点总数 | 2 | 48 个入口 |
| **总分** | **11** | **复杂** |

**文档策略**：按业务域拆分流程/规则/状态机文档，其他单份。预计 12 份文档。
```
...
- [ ] 至少 5 条业务规则有触发条件 + 执行动作 + 业务原因 + **来源类型**（后端校验/前端校验/数据库约束/业务常量/AOP拦截器），且**至少覆盖 3 种来源类型**（不能只从后端校验提取）
  > 🔧 **全量深挖模式 B**：8 路全扫到的所有业务规则都进文档，不限 5 条
- [ ] 角色矩阵覆盖所有识别到的用户类型
...
### 按业务域拆分文档（复杂项目，Phase 1 自动判断触发）

> 触发条件：Phase 1 复杂度评估总分 ≥ 8（见上方「项目复杂度自动判断标准」）。AI 自动判断，不需用户指定。

当项目复杂度评估为"复杂"时，**不要塞进一份"核心业务流程"文档**，按业务域拆分：
...
**自动判断标准**（取代原来"主线 4+ 条"的单维度判断）：
- Phase 1 复杂度总分 0-7 → 1 份"核心业务流程"文档
- Phase 1 复杂度总分 8-12 → **按业务域拆分多份**（每域 1 份 + 1 份跨域总览）
