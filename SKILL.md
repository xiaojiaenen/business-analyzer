...
**文档导航首页**：全部文档生成完毕后，创建 `business-docs/index.html`。风格与文章主题协调（暖纸白底 + 衬线标题 + 低饱和色），不需要 reacticle。

**index.html 必须包含**：
- 项目名称（衬线大字）+ 一句话简介
- 📖 推荐阅读顺序（编号列表）
- 📄 文档卡片（每篇一条：标题 + 概要 + 链接）
- ← 每篇文档顶部的"返回文档导航"链接指向此页

**index.html 色调参考**（与 press/tufte 主题协调）：

```html
<style>
  body { background:#fdfcf9; color:#3d3935; font-family:"Source Serif 4",Georgia,"Noto Serif SC","宋体",serif; }
  .container { max-width:680px; margin:0 auto; padding:64px 24px; }
  h1 { font-size:2.2rem; font-weight:600; letter-spacing:-0.02em; }
  .subtitle { color:#8b8580; font-size:1.05rem; margin:8px 0 48px; }
  h2 { font-size:1rem; font-weight:600; color:#6b6560; margin:40px 0 12px; letter-spacing:0.04em; text-transform:uppercase; }
  ol { padding-left:20px; color:#5b5550; }
  ol li { margin-bottom:4px; }
  .card { display:block; padding:20px 24px; margin-bottom:10px; background:#fff; border:1px solid #e7e3de; border-radius:4px; text-decoration:none; color:inherit; }
  .card:hover { border-color:#c4bdb4; }
  .card h3 { font-size:1.05rem; font-weight:600; margin:0 0 4px; }
  .card p { font-size:.9rem; color:#8b8580; margin:0; }
  footer { margin-top:56px; padding-top:14px; border-top:1px solid #e7e3de; font-size:.78rem; color:#b5aea6; }
</style>
```
